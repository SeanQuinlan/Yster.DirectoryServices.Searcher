function Clear-DSSAccountExpiration {
    <#
    .SYNOPSIS
        Clears the Account Expiration field for an Active Directory account.
    .DESCRIPTION
        Clears the Account Expiration attribute on a computer, user or service account in Active Directory.
    .EXAMPLE
        Clear-DSSAccountExpiration -SAMAccountName 'Guest'

        Clear the account expiration date on the "Guest" account.
    .NOTES
        References:
        https://docs.microsoft.com/en-us/powershell/module/addsadministration/clear-adaccountexpiration
    #>
    ...
    try {
        ...
        $PSBoundParameters['AccountExpirationDate'] = $null
