function Remove-DSSContact {
    <#
    .SYNOPSIS
        Removes a specific contact object from Active Directory.
    .DESCRIPTION
        Queries Active Directory for a specific contact object and then deletes it, based on one of the following specified parameters:
            - DistinguishedName
            - ObjectSID (SID)
            - ObjectGUID (GUID)
    .EXAMPLE
        Remove-DSSContact -ObjectGUID 'b675195d-e637-447e-ad26-baded4df1ba0'

        Deletes the contact with the above GUID.
    .NOTES
        References:
    ...
    try {
        ...
        Remove-DSSObjectWrapper -ObjectType 'Contact' -BoundParameters $PSBoundParameters
